...
public enum AIContentPart: Codable, Sendable, Equatable {
    ...
    public func encode(to encoder: any Encoder) throws {
        ...
        case .image(let id): try c.encode("image", forKey: .type); try c.encode(id, forKey: .fileId)
        case .file(let id): try c.encode("file", forKey: .type); try c.encode(id, forKey: .fileId)
        }
    }
}
...
public enum AIStreamEvent: Sendable, Equatable {
    ...
    public init(wire: JSONValue) throws {
        ...
        case "response.requires_action": self = .requiresAction(try Self.require(wire, "response").decode())
    ...
    private static func require(_ wire: JSONValue, _ key: String) throws -> JSONValue {
        guard let value = wire[key] else { throw MMGTError.invalidResponse("Missing AI event field: \(key)") }
        return value
    }
}
