...
public struct BillingClient: Sendable {
    ...
    public init(configuration: ServiceConfiguration, tokenProvider: @escaping AccessTokenProvider, transport: any HTTPTransport = URLSessionTransport()) {
        self.configuration = configuration
        http = HTTPClient(configuration: configuration, tokenProvider: tokenProvider, transport: transport)
    }
    private func request<T: Decodable & Sendable>(_ path: [String], method: String = "GET", body: JSONValue? = nil, authenticated: Bool = true) async throws -> T {
        try await http.request(path: ["app", configuration.appID] + path, method: method, body: body, authenticated: authenticated)
    }
    public func getCatalog() async throws -> BillingCatalogResponse { try await request(["catalog"], authenticated: false) }
    ...
    public func startWorkspaceCheckout(_ input: WorkspaceCheckoutRequest) async throws -> CheckoutResponse {
    ...
    public func changeSubscription(_ input: ChangeSubscriptionRequest) async throws -> ChangeSubscriptionResponse {
    ...
    public func listWorkspaces() async throws -> WorkspaceListResponse { try await request(["workspaces"]) }
    public func getWorkspace(_ id: String) async throws -> WorkspaceResponse { try await request(["workspaces", id]) }
    ...
    public func createInvites(workspaceID: String, input: CreateInvitesRequest) async throws -> CreateInvitesResponse {
        try await request(["workspaces", workspaceID, "invites"], method: "POST", body: .encoding(input))
    }
    ...
    public func resendInvite(workspaceID: String, inviteID: String) async throws -> ResendInviteResponse {
