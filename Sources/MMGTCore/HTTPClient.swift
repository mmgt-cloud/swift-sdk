...
public struct APIError: Error, Sendable, Equatable {
    ...
    public init(status: Int, code: String, message: String, requestID: String? = nil, retryAfter: String? = nil, body: JSONValue? = nil) {
        self.status = status; self.code = code; self.message = message
        self.requestID = requestID; self.retryAfter = retryAfter; self.body = body
    }
}
...
public final class URLSessionTransport: HTTPTransport, Sendable {
    ...
    public func send(_ request: URLRequest) async throws -> HTTPResponse {
        do {
            ...
            guard let http = response as? HTTPURLResponse else { throw MMGTError.invalidResponse("Expected HTTP response") }
...
public struct HTTPClient: Sendable {
    ...
    public init(configuration: ServiceConfiguration, tokenProvider: AccessTokenProvider? = nil, transport: any HTTPTransport = URLSessionTransport()) {
        self.configuration = configuration; self.tokenProvider = tokenProvider; self.transport = transport
    }
    ...
    public func request<T: Decodable & Sendable>(
        ...
        let data = try await send(path: path, method: method, data: body.map { try JSONEncoder().encode($0) }, query: query, authenticated: authenticated, headers: headers)
        do { return try JSONDecoder().decode(type, from: data.isEmpty ? Data("{}".utf8) : data) }
        catch { throw MMGTError.invalidResponse("Response did not match \(T.self)") }
    }

    public func send(path: [String], method: String = "GET", data: Data? = nil,
        ...
        request.httpMethod = method; request.httpBody = data; request.timeoutInterval = 60
        ...
        if authenticated {
            guard let token = try await tokenProvider?(), !token.isEmpty else { throw MMGTError.unauthenticated }
        ...
        guard (200..<300).contains(response.status) else {
            ...
            throw APIError(status: response.status,
                           code: detail?["code"]?.string ?? body?["code"]?.string ?? detail?.string ?? "http_error",
                           message: detail?["message"]?.string ?? body?["message"]?.string ?? detail?.string ?? "Request failed",
                           requestID: response.headers["x-request-id"], retryAfter: response.headers["retry-after"], body: body)
        }
