...
public struct RealtimeCursor: Codable, Sendable, Equatable {
  ...
  public init(revision: String = "0", eventID: String? = nil) {
...
public protocol RealtimeCursorStore: Sendable {
  ...
  func compareAndSet(
    identity: AccountIdentity, channel: String, expected: RealtimeCursor, eventID: String?
  ) async throws -> RealtimeCursor?
}

public actor MemoryRealtimeCursorStore: RealtimeCursorStore {
  private var values: [AccountIdentity: [String: RealtimeCursor]] = [:]
  public init() {}
  public func load(identity: AccountIdentity, channel: String) -> RealtimeCursor {
    values[identity]?[channel] ?? .init()
  }
...
public final class FileRealtimeCursorStore: RealtimeCursorStore, Sendable {
  ...
  public init(fileURL: URL) throws {
    guard fileURL.isFileURL else {
      throw MMGTError.invalidConfiguration("Cursor store requires a local URL")
    }
  ...
  private func locked<T>(_ body: (inout [String: RealtimeCursor]) throws -> (T, Bool)) throws -> T {
    ...
    while flock(descriptor, LOCK_EX) != 0 {
      if errno != EINTR { throw CocoaError(.fileWriteUnknown) }
    }
    defer { flock(descriptor, LOCK_UN) }
    var values: [String: RealtimeCursor] = [:]
    if FileManager.default.fileExists(atPath: file.path) {
      values = try JSONDecoder().decode([String: RealtimeCursor].self, from: Data(contentsOf: file))
    }
    let (result, changed) = try body(&values)
    if changed {
      try JSONEncoder().encode(values).write(to: file, options: [.atomic, .completeFileProtection])
    }
    return result
  }
