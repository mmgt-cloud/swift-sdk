#!/usr/bin/env python3
"""Run real guest Keychain CAS in an ad-hoc signed Simulator application host."""
import argparse
import datetime
import json
...
folder = root / '.artifacts' / ('keychain-' + stamp)
...
command = ['xcodebuild', 'test', '-project', 'Examples/MMGTExample/MMGTExample.xcodeproj',
           ...
           '-only-testing:MMGTDeviceTests/GuestKeychainTests',
...
report = dict(schemaVersion=1, testCounts=counts, startedAt=stamp, destination=args.destination,
              ...
              scope='Real Security Keychain CAS in a synthetic Simulator application; physical device and environment acceptance remain separate')
